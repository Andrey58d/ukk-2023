...
if(isset($_POST['ngadu'])){
    ...
    if(!empty($foto)){
        if(in_array($ekstensi, $ekstensi_diperbolehkan) === true){
            $q = "insert into pengaduan (id_pengaduan, tgl_pengaduan, nik, isi_laporan, foto, status) values ('', '$tgl', '$nik', '$pengaduan', '$foto', '0')";
            $r = mysqli_query($con, $q);
            if($r){
                move_uploaded_file($file_tmp, '../../asset/img/masyarakat' . $foto);
            }
        }
    }else{
        $q = mysqli_query($con, "insert into pengaduan (id_pengaduan, tgl_pengaduan, nik, isi_laporan, foto, status) values ('', '$tgl', '$nik', '$pengaduan', '$foto', '0')");
    }
}
                ...
                ?> 
                    <tr>
                        ...
                        <?php 
                            if(!empty($m -> foto)){
                                ?> 
                                    <td><img src="../../asset/img/masyarakat/<?= $m -> foto ?>" style="max-height: 100px;"></td>
                                <?php
                            }else{
                                ?> 
                                    <td><img alt="foto kosong"></td>
                                <?php
                            }
                        ?>
                        <td><?= $m -> status ?></td>
    ...
    ?> 
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    <button class="btn btn-success" name="tanggap">Kirim TAnggapan</button>
                </div>
                </form>
            </div>
            <div class="body-footer">
